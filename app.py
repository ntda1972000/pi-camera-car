...
# ---------------------------------------------------------------------------
# MEDIAMTX (HLS / RTSP via hardware H.264) — optional
# Binary must live at <project_dir>/mediamtx
# ---------------------------------------------------------------------------
import subprocess as _subprocess
import signal as _signal

MEDIAMTX_BIN  = os.path.join(os.path.dirname(__file__), "mediamtx")
MEDIAMTX_CONF = os.path.join(os.path.dirname(__file__), "mediamtx.yml")
MEDIAMTX_HLS_PORT  = 8888
MEDIAMTX_RTSP_PORT = 8554

# Bitrate map (bits/sec) for rpiCamera — scales with resolution
_MEDIAMTX_BITRATE = {
    (160, 120): 150_000,
    (320, 240): 300_000,
    (480, 360): 600_000,
...
HLS_AVAILABLE = os.path.isfile(MEDIAMTX_BIN) and os.access(MEDIAMTX_BIN, os.X_OK)
if HLS_AVAILABLE:
...
def _write_mediamtx_conf():
    """Write a fresh mediamtx.yml with values from current settings."""
    w, h   = tuple(settings["resolution"])
    fps    = settings["fps"]
    idr    = max(fps * 2, 30)  # keyframe every 2s
    bps    = _MEDIAMTX_BITRATE.get((w, h), 300_000)
    rot    = settings.get("camera_rotation", 0)
    # rpicam-vid --rotation only accepts 0 or 180
    rpicam_rot = 180 if rot in (180,) else 0
    content = f"""# Auto-generated by pi-camera-car app.py — do not hand-edit while running
api: yes
...
def start_mediamtx():
    ...
    with _mediamtx_lock:
        ...
        try:
            ...
            logging.info(f"mediamtx started (pid {_mediamtx_proc.pid})")
        except Exception as e:
...
def api_status():
    ...
    mode = settings.get("stream_mode", "mjpeg")
    hls_url = (
        f"http://{request.host.split(':')[0]}:{MEDIAMTX_HLS_PORT}/cam/index.m3u8"
        if mode == "hls" else None
    )
    return jsonify({
        "resolution": settings["resolution"],
        "fps": settings["fps"],
        ...
        "mb_per_hour": round(mb_per_hour, 2),
        "camera_ok": (
            is_mediamtx_running() if mode == "hls"
            else (elapsed is not None and elapsed < WATCHDOG_TIMEOUT)
        ),
...
@app.route("/api/hls_info")
def api_hls_info():
    """Return HLS/RTSP URLs for the current stream."""
    host = request.host.split(":")[0]
    return jsonify({
        "hls_url":  f"http://{host}:{MEDIAMTX_HLS_PORT}/cam/index.m3u8",
        "rtsp_url": f"rtsp://{host}:{MEDIAMTX_RTSP_PORT}/cam",
        "webrtc_url": f"http://{host}:8889/cam",
        "running": is_mediamtx_running(),
    })
